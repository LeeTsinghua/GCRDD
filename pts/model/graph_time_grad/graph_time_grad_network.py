...
from .graph_gaussian_diffusion import GraphGaussianDiffusion

class GraphTimeGradTrainingNetwork(nn.Module):
    @validated()
    def __init__(
        self,
        input_size: int,
        ...
        **kwargs,
        ...
        rnn_cls = {"LSTM": GConvGRU, "GRU": nn.GRU}[cell_type]
        self.rnn = rnn_cls(
        ...
        self.denoise_fn = GraphEpsilonTheta(
            num_nodes=num_nodes,
    ...
    def get_lagged_subsequences(
        ...
        for lag_index in indices:
            ...
            lagged_values.append(sequence[:, begin_index:end_index, ...].unsqueeze(1))   # why leave the last one out
    ...
    def unroll(
        ...
        # index_embeddings = self.embed(target_dimension_indicator)   # Dont need dimension indicator
        ...
        # unroll encoder
        # outputs, state = self.rnn(inputs, edge_index, edge_weight, begin_state)
        outputs, state = self.rnn(inputs.reshape(inputs.shape[0], inputs.shape[1],self.num_nodes*11), begin_state)
    ...
    def forward(
        ...
        edge_indices = edge_index + (self.num_nodes * torch.arange(0, edge_index.shape[0])).to(edge_weight.get_device()).unsqueeze(1).unsqueeze(2)
...
class GraphTimeGradPredictionNetwork(GraphTimeGradTrainingNetwork):
    ...
    def sampling_decoder(
        ...
        for k in range(self.prediction_length):
            ...
            new_samples = self.diffusion.sample(edge_index, edge_weight, cond=distr_args)
